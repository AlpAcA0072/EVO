...
def evaluate(ofa_net, dl, sdl, scale, subnets):
    ...
    for subnet in subnets:
        ...
        ofa_net.module.cp.backbone.set_active_subnet(d=subnet['depth'], e=subnet['expand'])
...
def main():
    ...
    exp_dir = 'ofa_plus_inverted_'

    if args.task == 'depth':
        exp_dir += 'depth@phase{}'.format(args.phase)
        dynamic_batch_size = 2

        expand_constr, width_constr = [6], [2]
        if args.phase == 1:
            depth_constr = [3, 4]
            lr_start = 2.5e-3
            warmup_steps = 5
        ...
        subnets2eval = [
            {'depth': depth_constr[0], 'expand': 6},
            {'depth': depth_constr[-1], 'expand': 6}]

    elif args.task == 'expand':
        exp_dir += 'depth-expand@phase{}'.format(args.phase)
        dynamic_batch_size = 4

        depth_constr, width_constr = [2, 3, 4], [2]
        if args.phase == 1:
            expand_constr = [4, 6]
            lr_start = 2.5e-3
            warmup_steps = 5
        ...
        subnets2eval = [
            {'depth': 2, 'expand': expand_constr[0]},
            {'depth': 2, 'expand': expand_constr[-1]},
            {'depth': 4, 'expand': expand_constr[0]},
            {'depth': 4, 'expand': expand_constr[-1]}]
