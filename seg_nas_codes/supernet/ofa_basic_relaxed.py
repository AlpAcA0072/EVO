...
class OFABasicBlockNets(ResNets):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0,
        ...
        super(OFABasicBlockNets, self).__init__(input_stem, blocks, classifier)
    ...
    def config(self):
        return {
            'name': OFABasicBlockNets.__name__,
    ...
    def load_state_dict(self, state_dict, **kwargs):
        ...
        super(OFABasicBlockNets, self).load_state_dict(model_dict)
    ...
    def set_active_subnet(self, d=None, e=None, w=None, **kwargs):
        ...
        for stage_id, (block_idx, d) in enumerate(zip(self.grouped_block_index, depth)):
            if d is not None:
                # self.runtime_depth[stage_id] = max(self.depth_list) - d
                self.runtime_depth[stage_id] = self.depth_list[stage_id] - d
            if width_mult is not None:
                for idx in block_idx:
                    self.blocks[idx].active_out_channel = self.blocks[idx].out_channel_list[width_mult[idx + 1]]
    ...
    def sample_active_subnet(self):
        ...
        # every layer in every stage can has its own output # of channels
...
if __name__ == '__main__':
    ...
    ofa_network.set_constraint([2, 3, 4, 2], constraint_type='depth')
