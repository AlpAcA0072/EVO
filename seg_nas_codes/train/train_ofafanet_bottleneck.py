...
def main():
    ...
    exp_dir = 'ofa_plus_bottleneck_'

    if args.task == 'depth':
        exp_dir += 'depth@phase{}'.format(args.phase)
        dynamic_batch_size = 2

        expand_constr, width_constr = [0.35], [2]
        if args.phase == 1:
            depth_constr = [0, 1]
            lr_start = 2.5e-3
            warmup_steps = 5
        ...
        subnets2eval = [
            {'depth': depth_constr[0], 'expand': 0.35, 'width': 2},
            {'depth': depth_constr[-1], 'expand': 0.35, 'width': 2}]

    elif args.task == 'expand':
        exp_dir += 'depth-expand@phase{}'.format(args.phase)
        dynamic_batch_size = 4

        depth_constr, width_constr = [0, 1, 2], [2]
        if args.phase == 1:
            expand_constr = [0.25, 0.35]
            lr_start = 2.5e-3
            warmup_steps = 5
            max_iter = 8000
            save_iter_start = 6000
        ...
        subnets2eval = [
            {'depth': 0, 'expand': expand_constr[0], 'width': 2},
            {'depth': 0, 'expand': expand_constr[-1], 'width': 2},
            {'depth': 2, 'expand': expand_constr[0], 'width': 2},
            {'depth': 2, 'expand': expand_constr[-1], 'width': 2}]

    elif args.task == 'width':
        exp_dir += 'depth-expand-width@phase{}'.format(args.phase)
        dynamic_batch_size = 4

        depth_constr, expand_constr = [0, 1, 2], [0.2, 0.25, 0.35]
        if args.phase == 1:
            width_constr = [1, 2]
            lr_start = 2.5e-3
            warmup_steps = 5
        ...
        subnets2eval = [
            {'depth': 0, 'expand': 0.2, 'width': width_constr[0]},
            {'depth': 0, 'expand': 0.2, 'width': width_constr[-1]},
            {'depth': 0, 'expand': 0.35, 'width': width_constr[0]},
            {'depth': 0, 'expand': 0.35, 'width': width_constr[-1]},
            {'depth': 2, 'expand': 0.2, 'width': width_constr[0]},
            {'depth': 2, 'expand': 0.2, 'width': width_constr[-1]},
            {'depth': 2, 'expand': 0.35, 'width': width_constr[0]},
            {'depth': 2, 'expand': 0.35, 'width': width_constr[-1]}]
