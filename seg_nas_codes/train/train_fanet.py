...
def parse_args():
    ...
    parse.add_argument('--backbone', dest='backbone', type=str, default=None, )
...
def train():
    args = parse_args()
    save_pth_path = osp.join(args.respath, 'ofa_fanetplus_r152d')
    ...
    # -------------------------- this is for training the full-capacity OFA-FANet ------------------------ #
    # ------------------------- basic block ----------------------- #
    # net = OFAFANet(
    ...
    # net.load_backbone_state_dict(args.backbone)
    # net.set_active_subnet(d=[2, 3, 4, 2], e=[1.0] * 16, w=[2] * 5)
    ...
    #     output_aux=True)
    # net.load_backbone_state_dict(args.backbone)
    # net.set_active_subnet(d=2, e=0.35, w=2)
    ...
    # net.load_backbone_state_dict(args.backbone)
    # net.set_active_subnet(d=4, e=6, w=1.0)
    ...
    # -------------------------------------------------- #
    # for training a regular FANet with different timm model backbone
    net = FANetPlus(backbone='resnet152d', feature_dim=(512, 1024, 2048), output_aux=True)
